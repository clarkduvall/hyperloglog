...
func TestHLLPPEstimateBias(t *testing.T) {
	...
	if math.Abs(b - 7.5988) > 0.00001 { t.Error(b) }

	h, _ = NewHyperLogLogPlus(16)
	b = h.estimateBias(55391.4373)
	if math.Abs(b - 39416.9373) > 0.00001 { t.Error(b) }
}
...
func TestHLLPPMerge(t *testing.T) {
	...
	h.Add(fakeHash64(k1))
	if !h.tmpSet[h.encodeHash(k1)] { t.Error("key not in hash") }

	k2 := uint64(0x000fff8f00000000)
	h.Add(fakeHash64(k2))
	if !h.tmpSet[h.encodeHash(k2)] { t.Error("key not in hash") }

	if len(h.tmpSet) != 2 { t.Error(h.tmpSet) }

	h.merge()
	if len(h.tmpSet) != 0 { t.Error(h.tmpSet) }
	if h.sparseList.Count != 2 { t.Error(h.sparseList) }

	iter := h.sparseList.Iter()
	n := iter.Next()
	if n != h.encodeHash(k2) { t.Error(n) }
	n = iter.Next()
	if n != h.encodeHash(k1) { t.Error(n) }

	k3 := uint64(0x0f00017000000000)
	h.Add(fakeHash64(k3))
	if !h.tmpSet[h.encodeHash(k3)] { t.Error("key not in hash") }

	h.merge()
	if len(h.tmpSet) != 0 { t.Error(h.tmpSet) }
	if h.sparseList.Count != 3 { t.Error(h.sparseList) }

	iter = h.sparseList.Iter()
	n = iter.Next()
	if n != h.encodeHash(k2) { t.Error(n) }
	n = iter.Next()
	if n != h.encodeHash(k3) { t.Error(n) }
	n = iter.Next()
	if n != h.encodeHash(k1) { t.Error(n) }
...
func TestHLLPPError(t *testing.T) {
	_, err := NewHyperLogLogPlus(3)
	if err == nil { t.Error("precision 3 should return error")}

	_, err = NewHyperLogLogPlus(18)
	if err != nil { t.Error(err)}

	_, err = NewHyperLogLogPlus(19)
